...
def _cell(value):
    ...
    if isinstance(value,dict):
        parts=[f"{k}={_cell(v)}" for k,v in value.items() if v not in (None,"",[],{})]
        return ", ".join(parts) if parts else "—"
    if isinstance(value,list):
        return ", ".join(_cell(x) for x in value) if value else "—"
    return str(value).replace("\n"," ")

def table(headers,rows):
    if not rows: return "_暂无可靠技术事实。_"
    return "| "+" | ".join(headers)+" |\n| "+" | ".join(["---"]*len(headers))+" |\n"+"\n".join("| "+" | ".join(_cell(c) for c in row)+" |" for row in rows)

def _source_area(path):
    if isinstance(path, dict):
        path = path.get('file') or ''
    parts=(path or '').split('/')
...
def dependency_summary(imports:list):
    ...
    for edge in imports:
        ...
        src=_source_area(source_ref)
        dst=_target_area(source_ref if isinstance(source_ref, str) else (source_ref.get('file') or ''), edge.get('specifier') or '')
...
def build_system(repo:Path):
    ...
    if zh:
        title="技术分析附录（机器事实）"; generated="这是给 Agent、审计和追踪使用的技术附录，不是人读迁移计划。请优先阅读 `docs/modernization/`。未获得可靠证据的字段使用 `—` 或明确说明，不代表已完成。"
        sections=[
        ...
        title="Technical Analysis Appendix (Machine Facts)"; generated="This is a technical appendix for agents, audit, and traceability. Human readers should start in `docs/modernization/`."
    ...
    out=repo/"migration/system/machine-analysis-report.md"
    out.write_text(body,encoding="utf-8")
    # Backward-compatible bridge: keep the historical path, but stop dumping the
    # technical 18-section report into the primary human-facing filename.
    bridge=repo/"migration/system/migration-master-report.md"
    if zh:
        bridge_body=f"""---
language: {lang}
...
1. `docs/modernization/project-overview.md` — 项目现状和主要业务区域
2. `docs/modernization/architecture-modernization-plan.md` — 真正的人读实施计划
3. `docs/modernization/progress.md` — 当前进度
4. `docs/modernization/domains/` — 已确认业务模块说明

需要查看 Stable ID、Route/API 原始事实、置信度和完整静态证据时，再阅读：

- `migration/system/machine-analysis-report.md`
- `migration/system/semantic-synthesis.yaml`
"""
    else:
        bridge_body=f"""---
language: {lang}
...
status: generated
---
# Modernization Report Entry

This path is kept for backward compatibility.

Start with `docs/modernization/project-overview.md`,
`docs/modernization/architecture-modernization-plan.md`, and
`docs/modernization/progress.md`.

Machine-level evidence is available in `migration/system/machine-analysis-report.md`.
"""
